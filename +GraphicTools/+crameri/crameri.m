...
figpath=strcat(fileparts(mfilename('fullpath')),filesep,'crameri7.0.png');

datapath=strcat(fileparts(mfilename('fullpath')),filesep,'CrameriColourMaps7.0.mat');

if nargin==0
   figure('menubar','none','numbertitle','off','Name','crameri options:')
   
   if license('test','image_toolbox')
      imshow(imread(figpath)); 
   else
      axes('pos',[0 0 1 1])
      image(imread(figpath)); 
      axis image off
   end
